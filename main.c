/* Name: main.c
 * Project: Atmega328P USB Bootloader
 * Author: Md. Faridul Islam
 *
 * Most of the code copied from "Christian Starkjohann" in his project "USBaspLoader"
 ...
 *
 * Features added:
 * 1. Automatic bootloader entry on POR
 * 2. Automatic bootloader entry on external reset
 * 3. Automatic bootloader exit on timeout (4s)
 * 4. LED Indicator for bootloader mode indication
 *
 */
...
/* LED indicator for boot mode*/
#define ONBOARD_LED_ENABLE
#define ONBOARD_LED_DDR  DDRD
#define ONBOARD_LED_PORT PORTD
#define ONBOARD_LED_bp   4  
...
int __attribute__((noreturn)) main(void)
{
    ...
    if(bootLoaderCondition()){
		...
		TCNT1 = 0;       /* Added for automatic timeout */
...
#endif
        }while(TCNT1<60000);  /* main event loop */
    }
    leaveBootloader();
}
